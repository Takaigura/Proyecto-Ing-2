...
public class HelloController implements Initializable {
	...
	private void BotonRegistrar(ActionEvent event) {
		...
		if (txtf_propietario.getText().isEmpty() || txtf_placa.getText().isEmpty()) {
			...
			alerta.setContentText("Propietario " + txtf_propietario.getText() + System.lineSeparator()
					+ "Placa del carro " + txtf_placa.getText());
		}
		alerta.showAndWait();
	...
	private void agregarAutosBD() {
		...
		try {
			...
			query.execute();
			// Añade los registros a la observable list desde la BD y esta lista
			// nueva/refrescada a la tabla
	...
	private void BotonModificar(ActionEvent event) {
		...
		if (p == null) {
			...
			System.out.println("Historial de Modificaciones: " + historial_list.toString());
