...
public class HelloController implements Initializable, Cloneable{
    @FXML
    ...
    private List<Auto> historial = new ArrayList<>();
    ...
    private void BotonRegistrar(ActionEvent event) {
        ...
        Auto p = new Auto(placa, propietario);
        historial.add(p.cloneAuto());
        System.out.println("Historial de ingresos: "+historial.toString());
        Alert alerta;
    ...
    private void BotonModificar(ActionEvent event) {
        ...
        if (p == null) {
            ...
            String placa = this.Placa.getText();
            String propietario = this.Propietario.getText();
            Auto mod = new Auto(placa, propietario);
